...
export async function saveDirectUploadToDb(projectId: string, url: string, type: string) {
  // Find current max order
  const maxOrderMedia = await prisma.projectMedia.findFirst({
    where: { projectId },
    orderBy: { order: "desc" },
  ...
  const currentOrder = maxOrderMedia ? maxOrderMedia.order + 1 : 0;

  await prisma.projectMedia.create({
    data: { url, type, projectId, order: currentOrder },
  });
